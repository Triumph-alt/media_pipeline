...
当前纯视频路径以首个有效帧 PTS 为时间零点，使用 `std::chrono::steady_clock` 控制帧呈现
节奏；该临时路径尚未接入最终的 Pipeline Clock。VideoRender 的窗口事件轮询和 RouteDelivery
ack 由 §5.3.1 与 SinkNode 通用循环负责，本节只展示时间调度相关部分：

```cpp
void VideoRenderNode::consume(const Buffer* buf) {
    // 上游 Delivery 已由 SinkNode 循环取得；窗口关闭轮询和最终 ack 在其他职责位置处理。
    if (!waitForPresentationTime(buf->pts)) {
        return;
    }

    // 当前阶段只支持紧密排列的 YUV420P；此处省略 plane 计算和 Texture 更新细节。
    SDL_UpdateYUVTexture(texture_, nullptr, y_plane, width,
                         u_plane, chroma_width, v_plane, chroma_width);
...
bool VideoRenderNode::waitForPresentationTime(int64_t pts_us) {
    // 以首帧 PTS 和 steady_clock 建立纯视频临时时间基准；等待期间检查 stop_requested_。
}
```
