...
int main(int argc, char* argv[]) {
    if (argc < 3 || argc > 8) {
        fprintf(stderr,
                "Usage: %s <device> <output.flv> [width] [height] [encoder] "
                "[framerate] [--overwrite]\n",
                argv[0]);
        return 1;
    }
    ...
    auto* mux = pipeline.addNode<pipeline::AVMuxNode>("flv_mux", pipeline::MuxFormat::FLV);
    ...
    if (!capture || !encode || !mux || !file ||
        ...
        !pipeline.build()) {
        fprintf(stderr, "V4L2 FLV recording pipeline setup failed\n");
        return 1;
    }
    if (!pipeline.play()) {
        fprintf(stderr, "V4L2 FLV recording start failed: %s\n",
                pipeline.lastError().c_str());
        return 1;
    }

    fprintf(stderr,
            "recording %s to %s as FLV through '%s'; press Ctrl+C to stop\n",
    ...
    if (!error.empty()) {
        fprintf(stderr, "V4L2 FLV recording failed: %s\n", error.c_str());
        return 1;
    }
    return 0;
}
