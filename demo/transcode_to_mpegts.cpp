...
int main(int argc, char* argv[]) {
    if (argc < 3 || argc > 6) {
        fprintf(stderr,
                "Usage: %s <input> <output.ts> [encoder] [framerate] [--overwrite]\n",
                argv[0]);
        return 1;
    }
    ...
    // 与 transcode_to_flv 同拓扑：视频 Decode→重编码，输入 AAC encoded 旁路进同一 MPEG-TS Mux
    ...
    if (!demux || !decode || !encode || !mux || !file ||
        ...
        !pipeline.build()) {
        fprintf(stderr, "MPEG-TS transcode pipeline setup failed\n");
        return 1;
    }
    if (!pipeline.play()) {
        fprintf(stderr, "MPEG-TS transcode start failed: %s\n", pipeline.lastError().c_str());
        return 1;
    }
    ...
    if (!error.empty()) {
        fprintf(stderr, "MPEG-TS transcode failed: %s\n", error.c_str());
        return 1;
    }

    fprintf(stderr, "MPEG-TS transcode completed with natural EOS: %s\n",
