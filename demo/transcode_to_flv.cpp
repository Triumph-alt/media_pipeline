...
int main(int argc, char* argv[]) {
    if (argc < 3 || argc > 6) {
        fprintf(stderr,
                "Usage: %s <input> <output.flv> [encoder] [framerate] [--overwrite]\n",
                argv[0]);
        return 1;
    }
    ...
    if (!demux || !decode || !encode || !mux || !file ||
        ...
        !pipeline.build()) {
        fprintf(stderr, "FLV transcode pipeline setup failed\n");
        return 1;
    }
    if (!pipeline.play()) {
        fprintf(stderr, "FLV transcode start failed: %s\n", pipeline.lastError().c_str());
        return 1;
    }
    ...
    if (!error.empty()) {
        fprintf(stderr, "FLV transcode failed: %s\n", error.c_str());
        return 1;
    }

    fprintf(stderr, "FLV transcode completed with natural EOS: %s\n",
